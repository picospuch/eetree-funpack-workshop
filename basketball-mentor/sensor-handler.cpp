...
TfLiteStatus SetupSensor(tflite::ErrorReporter* error_reporter) {
  // Switch on the IMU
  if (!IMU.begin()) {
  ...
  TF_LITE_REPORT_ERROR(error_reporter, "basketball-mentor starts!");

  return kTfLiteOk;
}

bool ReadSensor(tflite::ErrorReporter* error_reporter, float* input,
                       int length) {
  ...
  while (IMU.accelerationAvailable() && IMU.gyroscopeAvailable()) {
    ...
    if (!IMU.readAcceleration(x1, y1, z1)) {
      TF_LITE_REPORT_ERROR(error_reporter, "Failed to read data");
      break;
    }
    if (!IMU.readGyroscope(x2, y2, z2)) {
      TF_LITE_REPORT_ERROR(error_reporter, "Failed to read data");
      break;
    }
