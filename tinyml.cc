// not written in main.cc, because header conflict in global namespace, with m5stack's In_eSPI.h
#include "tinyml.h"
...
static tinyml_result_callback g_result_callback;

float maxValue = 0;
const char *maxLabel = 0;
static void display_results(ei_impulse_result_t* result)
{
  ...
  const char *label;
    ...
    g_result_callback(maxLabel, maxValue);
...
void tinyml_start_impulse(tinyml_result_callback result_callback, bool continuous, bool debug, bool use_max_uart_speed)
{
  g_result_callback = result_callback;
